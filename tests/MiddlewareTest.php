...
use Wistrix\Onboard\Tests\Stubs\UserOnboarded;

beforeEach(function () {
    $this->user = new User;

    Route::middleware(UserOnboarded::class)->group(function () {
...
it('continues with complete steps', function () {
    ...
    $this->user->onboarding()
        ->register('onboarding.step1', fn() => true)
        ->register('onboarding.step2', fn() => true);
...
it('redirects with incomplete steps', function () {
    ...
    $this->user->onboarding()
        ->register('onboarding.step1', fn() => true)
        ->register('onboarding.step2', fn() => false);
...
it('redirects with completed steps to default route', function () {
    ...
    $response->assertStatus(302);
    $response->assertRedirectToRoute(UserOnboarded::DEFAULT_ROUTE);
});